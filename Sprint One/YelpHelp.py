...
class bar:                                          #superclass 
    def __init__(self,bar_data):
        ...
        for z in bar_data['transactions']:
            self.transactions_type_list = [z]
...
#define more attributes from web-scraping system
#use reviews and web-scraping to populate more info about 
#safety,coolness and amenities via keyword search

def main():
    print("YelpHelp file")
    #bar1_object = bar(location,radius)
    #bar1_object.display()

    #bar1_coolness_object = bar1_object.coolness()
    #bar1_coolness_object.display()

    #bar1_safety_object = bar1_object.safety()
    #bar1_safety_object.display()

    #bar1_amenities_object = bar1_object.amenities()
    #bar1_amenities_object.display()


    #call yelp help API with location,radius and type=bar as parameters
    #returns some dataset containing n number of bars, and all relevant attributes for each bar
